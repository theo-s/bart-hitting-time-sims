...
for (dgp in c("sum","high", "low", "piecewise", "tree", "lss")) {
  ...
  if (use_saved_data) {
    results <- read.csv(paste0("results/cached_results/exp3_coverage_",dgp,"_rmse.csv"))
  } else {
    ...
    write.csv(results, paste0("results/cached_results/exp3_coverage_",dgp,"_rmse.csv"), row.names = FALSE)
  }
  ...
  ggsave(plot_i,filename = paste0("results/figures/exp3/",dgp,"coverage.pdf"), height = 2.5, width = 3)
...
p_final = grid.arrange(all_plots[[1]]+theme(legend.position ="none"),
                       ...
                       widths =c(4,4,5.5),
                       ncol = 3)
ggsave(p_final,filename = paste0("results/figures/exp3/all_coverages.pdf"), height = 5, width = 8)
